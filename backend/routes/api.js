...
module.exports = (pool) => {
      ...
      const [userResult] = await pool.query(
        `INSERT INTO usuarios_google (google_id, email, nome, access_token, refresh_token, token_expiry)
         ...
         ON DUPLICATE KEY UPDATE
           ...
           refresh_token = COALESCE(VALUES(refresh_token), usuarios_google.refresh_token),
  ...
  // Middleware para garantir autenticação Google
  ...
  router.post('/drive/export-page', ensureGoogleAuth, async (req, res) => {
    ...
    try {
      ...
      await pool.query(
        'INSERT INTO storage_references (caderno_id, pagina_id, folder_id, file_id, link) VALUES (?, ?, ?, ?, ?)',
  ...
  // Verificar status de exportação de uma página
  ...
  // Criar/obter pasta do caderno no Drive
